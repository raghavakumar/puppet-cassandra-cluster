...
class cassandra {
 ...
 exec { "download apache-cassandra-${cassandra::params::version}-bin.tar.gz":
        command => "wget ${cassandra::params::download_url}",
        cwd     => "${cassandra::params::cassandra_base}",
        creates =>"${cassandra::params::cassandra_base}/apache-cassandra-${cassandra::params::version}",
        require => File["cassandra-base"],
        user => "${cassandra::params::cassandra_owner}",                                                                                                          
      }
 ...
 exec { "untar apache-cassandra-${cassandra::params::version}-bin.tar.gz":
         cwd => "${cassandra::params::cassandra_base}", 
         command => "tar zxvf apache-cassandra-${cassandra::params::version}-bin.tar.gz",
         creates => "${cassandra::params::cassandra_base}/cassandra-${cassandra::params::version}",
         alias   => "untar-cassandra",
         refreshonly => true,
         subscribe   => File["cassandra-base"],
         user => "${cassandra::params::cassandra_owner}",
